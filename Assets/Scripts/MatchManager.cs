...
/// detection/elimination, and win conditions all live here.
///
...
/// real networking later means adding PhotonView/RPCs around RegisterPlayer/Eliminate/the timer
/// sync, not rebuilding the rules.
///
...
/// fill the Human roster locally so detection/elimination/timer logic is actually exercisable.
...
public class MatchManager : MonoBehaviour
{
    ...
    [SerializeField] private float touchEliminationDistance = 1.2f;
    ...
    public int HumansAlive => players.Count(p => p.Role == PlayerRole.Human && !p.Eliminated);
    public int HumansEliminated => players.Count(p => p.Role == PlayerRole.Human && p.Eliminated);
    ...
    private void Update()
    {
        ...
        var zombie = players.FirstOrDefault(p => p.Role == PlayerRole.Zombie && !p.Eliminated);
        if (zombie != null && zombie.Transform != null)
        {
            foreach (var human in players)
            {
                if (human.Role != PlayerRole.Human || human.Eliminated || human.Transform == null) continue;
                ...
                human.Detected = dist <= radius;

                if (dist <= touchEliminationDistance)
                    Eliminate(human);
            }
        }
    ...
    private void Eliminate(PlayerRecord human)
    {
        if (human.Eliminated) return;
        human.Eliminated = true;
        if (human.Transform != null) human.Transform.gameObject.SetActive(false);
    }
